---
headerVersion: 2023.11.25
lintedAt: '2026-08-22T23:22:55-04:00'
lintVersion: '3.4'
tags: [person]
species: human
ancestry: Addermarian
campaignInfo:
- {campaign: adma, type: met, date: 1715-05-17}
gender: female
born: 1662
name: Helen of Haldrenn
pronunciation: HEL-en of HAL-dren
affiliations:
- {org: Haldrenn, title: temple administrator of the temple to the mother, type: member}
whereabouts:
- {type: home, location: Haldrenn}
knownTo: [adma]
dm_owner: none
dm_notes: none
POV: 1715
title: Helen of Haldrenn
hide_toc: true
---
# Helen of Haldrenn
:speaker:{ .middle } *(HEL-en of HAL-dren)*  
<div class="grid cards ext-narrow-margin ext-one-column" markdown>
- :octicons-info-24:{ .lg .middle } __Biographical Information__

    An [Addermarian](<../../gazetteer/greater-sembara/addermarch/addermarch.md>) [human](<../../creatures/species/humans.md>) (she/her)  
    Born DR 1662 (88 years old)  
    Temple Administrator of the Temple to the Mother of [Haldrenn](<../../gazetteer/greater-sembara/addermarch/haldrenn.md>)  
    { .bio }

    Based in [Haldrenn](<../../gazetteer/greater-sembara/addermarch/haldrenn.md>), [Highmoor](<../../gazetteer/greater-sembara/addermarch/highmoor.md>), [Carlinshire](<../../gazetteer/greater-sembara/addermarch/carlinshire.md>)
</div>



:octicons-location-24:{ .lg .middle } Met by the [Addermarch Mercenaries](<../pcs/addermarch/addermarch-mercenaries.md>) on May 17th, 1715 in [Haldrenn](<../../gazetteer/greater-sembara/addermarch/haldrenn.md>), [Highmoor](<../../gazetteer/greater-sembara/addermarch/highmoor.md>), [Carlinshire](<../../gazetteer/greater-sembara/addermarch/carlinshire.md>)  


Helen is the temple administrator for the small temple to [The Mother](<../../gods-and-religions/gods/incorporeal-gods/mos-numena-pantheon/the-mother.md>) in [Haldrenn](<../../gazetteer/greater-sembara/addermarch/haldrenn.md>), and a collector of fey lore. She presides over community rites and maintains oral histories of fey activity in the [Highmoor](<../../gazetteer/greater-sembara/addermarch/highmoor.md>), including tales of the [Briarheart](<../extraplanar-powers/archfey/morlaith.md>).
## Events
- May 17, 1715 DR: Presided over the funeral rites for [Olay](<olay.md>), which were interrupted by an [worg attack](<../../campaigns/addermarch-campaign/addermarch-session-12.md>).





